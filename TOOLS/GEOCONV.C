 ...
 *
 * $Id: geoconv.c 1.24 1995/07/28 19:05:11 sam Exp $
 * $Locker:  $
 *
...
int main(int argc, char **argv)
{
	...
	BR_BANNER("GEOCONV","1994-1995","$Revision: 1.24 $");
...
"           -q Clear BR_MODF_QUICK_UPDATE\n");
				...
				case 'm': {
					...
					BrModelFindMany(NULL,mtable,nmodels);
				...
				case 'l': {
					int n;
					struct model_list_totals totals = { 0,0,0,0,0};
...
br_uint_32 BR_CALLBACK ModelList(br_model *model, struct model_list_totals *totals)
{
	printf(" %-16s %5d %5d  %5d %5d %5d %5d  %c%c%c%c %g\n",
		...
		model->nedges,
		model->nface_groups,
		model->nvertex_groups,
		model->nprepared_vertices,
	...
	if(totals) {
		...
		totals->face_groups += model->nface_groups;
		totals->vertex_groups += model->nvertex_groups;
		totals->sorted += model->nprepared_vertices;
	}
