...
void LoadTexture(char *file_name)
{
    ...
    int i,n;
    char *actual_file_name,*identifier;

    /* Load textures... */

    actual_file_name = strtok(file_name,",");
...
main(int argc,char *argv[])
{
...
#if 0
    setenv("BRENDER_DOS_GFX","VESA,W:640,H:480,B:8,C:0,M:0",0);
#endif
    {
    ...
    while (++argv,--argc) {
        ...
        if (!strcmp(argv[0],"-c")) {
            ...
            camera = BrActorSearch(root,argv[1]);
            if (camera==NULL)
        ...
        if (!strcmp(argv[0],"-l")) {
            ...
            light = BrActorSearch(root,argv[1]);
            if (light==NULL)
